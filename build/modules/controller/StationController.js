...
var StationContoller = {
  ...
  "loadStations": function loadStations() {
    $.get('' + ENDPOINTS.weatherstations + "/getAllStations", function (allStationsData) {
      // for each station
      for (var idx in allStationsData) {
        ...
        marker.addEventListener('click', function (station, evt) {
  ...
  "isActiveInTimeRange": function isActiveInTimeRange(station) {
    // idea: two time perions (A and B), two time points (0 = start, 1 = end)
    // A and B overlap if A0 < B1 and B0 < A1
    //                A0 < B1              B0 < A1
    if (station.min_year < UI.end && UI.start < station.max_year) return true;else return false;
  }
