...
var UI = {
  ...
  "updatePosition": function updatePosition(latOrig, lngOrig) {
  ...
  "saveToSvg": function saveToSvg(rootDivId, filename) {
  ...
  "saveToPng": function saveToPng(rootDivId, filename) {
    ...
    rootDiv.clone().attr("id", "clone").appendTo(body);
    ...
    clone.attr("width", width_png).attr("height", height_png).attr("viewbox", "0 0 " + width_png + " " + height_png).attr("preserveAspectRatio", "xMinYMin meet");
  ...
  "createCharts": function createCharts() {
    ...
    // stop if one of the values is not given
    if (isNaN(parseFloat(UI.lat)) || isNaN(parseFloat(UI.lng))) return null;
    ...
    if (UI.activeClimateCell) UI.loadDataForClimateCell();else if (UI.activeWeatherStation) WeatherStations.loadData();
  },

  "removeCharts": function removeCharts() {
    $('#climate-chart-wrapper').css('visibility', 'hidden');
    $('#plot-wrapper').css('visibility', 'hidden');
  },

  "loadDataForClimateCell": function loadDataForClimateCell() {
    // Draw the chart when all four ajax calls have received a response.
    $.when(getData(0), getData(1), getName(), getElevation()).done(function (a1, a2, a3, a4) {
      ...
      */
      ...
      for (var ds = 0; ds < 2; ds++) {
        ...
        for (var i = 0; i < len; i++) {
          ...
          // => convert to Celsius.
          if (ds == 0 && value >= 200) value -= 273.15;

          // Workaround for the precipitation dataset created by Uni Delaware.
          // Convert values from cm to mm.
          if (UI.dataset === "University of Delaware Air Temperature and Precipitation v4.01" && ds == 1) value *= 10;
      ...
      for (var monthIdx = 0; monthIdx < 12; monthIdx++) {
        ...
        for (var ds = 0; ds < 2; ds++) {
          var dataset = null;
          if (ds == 0) // temperature
            dataset = climateData.temp;else // precipitation
          ...
          // write data
          if (numValues > 0) dataset[monthIdx].mean = sum / numValues;
      ...
      for (var key in UI.catalog.dataset) {
        if (UI.catalog.dataset[key]._name == UI.dataset) {
          ...
          for (var name in UI.ncML[Index].variable) {
            if (UI.ncML[Index].variable[name]._shape == "time lat lon") variable = UI.ncML[Index].variable[name]._name;
          }url += "" + ENDPOINTS.thredds + "/ncss/" + UI.catalog.dataset[key].dataset[Index]._urlPath + "?var=" + variable + "&latitude=" + UI.lat + "&longitude=" + UI.lng + "&time_start=" + UI.start + "-01-01T00:00:00Z" + "&time_end=" + (UI.end - 1) + "-12-30T00:00:00Z";
        }
      }
    ...
    function getName() {
      var url = "" + ENDPOINTS.gazetteer + "/getName";
      return $.get(url, {
        lat: UI.lat,
        lng: UI.lng
    ...
    function getElevation() {
      var url = "" + ENDPOINTS.gazetteer + "/getElevation";
      return $.get(url, {
        lat: UI.lat,
        lng: UI.lng
      }).fail(function (jqXHR, textStatus, errorThrown) {
  ...
  "visualizeClimate": function visualizeClimate(climateData, geoname, elevation) {
    ...
    if (Math.max.apply(null, climateData.temp[0].rawData) < 100 && Math.min.apply(null, climateData.temp[0].rawData) > -100 && Math.max.apply(null, climateData.prec[0].rawData) < 10000 && Math.min.apply(null, climateData.prec[0].rawData) >= 0) {
      ...
      if (typeof geoname !== 'undefined') {
        if (geoname.name !== "" && typeof geoname.name !== "undefined") place = geoname.name + ", ";

        if (geoname.adminName1 !== "" && typeof geoname.adminName1 !== "undefined") admin = geoname.adminName1 + ", ";

        if (geoname.countryName !== "" && typeof geoname.countryName !== "undefined") country = geoname.countryName + ", ";

        title = place + admin + country;
      }

      // truncate the last ", "
      if (title.substring(title.length - 2, title.length) == ', ') title = title.substring(0, title.length - 2);
      ...
      saveButtonArea.innerHTML = "" + "<button id='save-chart-to-svg' class='btn btn-primary save-button'>SVG</button>" + "<button id='save-chart-to-png' class='btn btn-primary save-button'>PNG</button>";
      ...
      var saveButtonArea = document.createElement('div');
      saveButtonArea.className += "save-button-area ";
      saveButtonArea.innerHTML = "" + "<button id='save-plots-to-svg' class='btn btn-primary save-button'>SVG</button>" + "<button id='save-plots-to-png' class='btn btn-primary save-button'>PNG</button>";
  ...
  "listDatasets": function listDatasets() {
    var catalogUrl = "" + ENDPOINTS.thredds + "/catalog.xml";

    $.get(catalogUrl).done(function (data) {
      var x2js = new X2JS();
      ...
      for (var key in UI.catalog.dataset) {
        $("#datasets").append("<option " + "id='" + UI.catalog.dataset[key]._ID + "' value='" + UI.catalog.dataset[key]._name + "'>" + UI.catalog.dataset[key]._name + "</option>");
      }
      ...
      $.each(UI.catalog.dataset, function (index, value) {
        $("#source").append("<p> <b>" + UI.catalog.dataset[index]._name + ": </b> <br>" + UI.catalog.dataset[index].documentation[0].__text + ", <br>" + UI.catalog.dataset[index].documentation[1].__text + "</p>");
      });
  ...
  "setTimeFrame": function setTimeFrame(min, max) {
    $("#slider").slider({
      range: true,
      min: min,
      max: max,
      ...
      change: function change(event, ui) {
        ...
        // => reload!
            ...
            if (UI.activeClimateCell || UI.activeWeatherStation) UI.createCharts();else UI.removeCharts();
          }
      }
    });
  ...
  "setSliderLabels": function setSliderLabels() {
  ...
  "updateSlider": function updateSlider(min, max) {
  ...
  "getMetadata": function getMetadata() {
    $.each(UI.catalog.dataset, function (i, v) {
      if (v._name == $("#datasets").val()) {
        ...
        var urlTmp = "" + ENDPOINTS.thredds + "/ncml/" + v.dataset[0]._urlPath;
        var urlPre = "" + ENDPOINTS.thredds + "/ncml/" + v.dataset[1]._urlPath;

        $.when($.get(urlTmp), $.get(urlPre)).done(function (a1, a2) {

          var x2js = new X2JS();
          UI.ncML[0] = x2js.xml2json(a1[0]).netcdf;
          UI.ncML[1] = x2js.xml2json(a2[0]).netcdf;

          var metadata = "Spatial Resolution: " + UI.ncML[0].group[0].attribute[6]._value + "° x " + UI.ncML[0].group[0].attribute[7]._value + "°" + "<br>" + "Temporal Coverage: " + parseInt(v.timeCoverage.start) + " - " + parseInt(v.timeCoverage.end);

          $("#datasetInfo").empty();
          $("#datasetInfo").css("display", "block");
          $("#datasetInfo").html("<b>Data reference:</b><br>" + v.documentation[0].__text + ", <br> " + v.documentation[1].__text + "<br><br>" + metadata);

          if ($("#slider").is(":empty")) {
            UI.setTimeFrame(parseInt(v.timeCoverage.start), parseInt(v.timeCoverage.end));
          } else {
            UI.updateSlider(parseInt(v.timeCoverage.start), parseInt(v.timeCoverage.end));
          }
  ...
  "setDiagramTitle": function setDiagramTitle() {
  ...
  "resetSliderHandles": function resetSliderHandles() {
    UI.periodChange = {
      oldLeft: $("#slider").slider("values", 0),
      oldRight: $("#slider").slider("values", 1)
    };

    $("#slider").slider("values", [$("#slider").slider("values", 1) - 30, $("#slider").slider("values", 1)]);
  ...
  "activatePanning": function activatePanning() {
    ...
    if ($("#climate-chart-wrapper").width() < 500) {
      ...
      if ($('#reset').length === 0) {
        var reset = $("<button></button>").attr("id", "reset").attr("class", "btn btn-primary").text("Reset Chart");
        $("#save").prepend(reset);
